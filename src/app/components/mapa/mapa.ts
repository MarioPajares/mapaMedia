...
export class MapaComponent implements AfterViewInit, OnDestroy {
  ...
  private currentRaceDistanceM?: number;
  private activeDistancePopupM?: number;
  ...
  private async initializeMap(): Promise<void> {
    ...
    this.map.on('click', () => {
      this.activeDistancePopupM = undefined;
    });
  ...
  protected async deactivateGps(): Promise<void> {
    ...
    this.userMarker = undefined;
    this.currentRaceDistanceM = undefined;
    this.activeDistancePopupM = undefined;
    this.updateDistanceMarkers();
  ...
  private startSharedLocationListener(): void {
    ...
    this.sharedLocationUnsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        const location = snapshot.docs
          .map((locationDoc) => locationDoc.data() as Partial<SharedLocation>)
          .sort((first, second) => (second.capturedAtMs ?? 0) - (first.capturedAtMs ?? 0))[0];
  ...
  private updateDistanceMarkers(): void {
    ...
    if (this.currentRaceDistanceM === undefined) {
      return;
    }
    ...
    for (const marker of this.getDistanceMarkers(this.routePoints)) {
      const markerLayer = L.marker(marker.point, {
        ...
        icon: L.divIcon({
          className: marker.isFinish ? 'distance-marker finish-marker' : 'distance-marker',
          html: `<span>${marker.label}</span>`,
          iconSize: marker.isFinish ? [48, 30] : [30, 30],
          iconAnchor: marker.isFinish ? [24, 15] : [15, 15],
        }),
      }).bindPopup(this.getDistanceMarkerTooltip(marker), {
        autoClose: true,
        closeButton: false,
        closeOnClick: true,
        offset: [0, -12],
      ...
      markerLayer.on('click', (event) => {
        this.activeDistancePopupM = marker.distanceM;
        markerLayer.openPopup();
        L.DomEvent.stopPropagation(event);
      });
      markerLayer.addTo(this.map!);

      if (this.activeDistancePopupM === marker.distanceM) {
        markerLayer.openPopup();
      }
  ...
  private getDistanceMarkers(routePoints: RoutePoint[]): DistanceMarker[] {
    ...
    for (let markerIndex = 1; markerIndex <= markerCount; markerIndex += 1) {
      ...
      markers.push({
        distanceM: kilometer * 1000,
        estimatedTime: this.getEstimatedArrivalTime(kilometer),
        label: String(kilometer),
        point,
      });
    }

    const finishPoint = this.getPointAtDistance(routePoints, routeDistanceM);

    if (finishPoint) {
      markers.push({
        distanceM: targetDistanceM,
        estimatedTime: this.getEstimatedArrivalTimeAtDistance(targetDistanceM),
        label: 'Meta',
        point: finishPoint,
        isFinish: true,
      });
  ...
  private getDistanceMarkerTooltip(marker: DistanceMarker): string {
    const markerText = marker.isFinish ? 'Meta' : `Km ${marker.label}`;

    return `${markerText} · Est. ${marker.estimatedTime}`;
  }
  ...
  private getEstimatedArrivalTime(kilometer: number): string {
    return this.getEstimatedArrivalTimeAtDistance(kilometer * 1000);
  }

  private getEstimatedArrivalTimeAtDistance(distanceM: number): string {
    ...
    estimatedArrival.setSeconds(estimatedArrival.getSeconds() + this.getEstimatedElapsedSecondsAtDistance(distanceM));
  ...
  private getEstimatedElapsedSecondsAtDistance(distanceM: number): number {
    const elapsedSeconds = ESTIMATED_PACE_SECONDS_BY_KM
      .slice(0, Math.floor(distanceM / 1000))
      .reduce((total, paceSeconds) => total + paceSeconds, 0);
    const remainingDistanceM = distanceM % 1000;
    const paceIndex = Math.min(ESTIMATED_PACE_SECONDS_BY_KM.length - 1, Math.floor(distanceM / 1000));
    const partialSeconds = (remainingDistanceM / 1000) * ESTIMATED_PACE_SECONDS_BY_KM[paceIndex];

    return Math.round((elapsedSeconds + partialSeconds) / 60) * 60;
  }
  ...
  private showLocation(position: GeolocationPosition): void {
    ...
    if (this.lastLocationShownAt !== 0 && now - this.lastLocationShownAt < LOCATION_UPDATE_INTERVAL_MS) {
      return;
    }

    this.lastLocationShownAt = now;
    ...
    this.updateCurrentRaceDistance();
  ...
  private updateCurrentRaceDistance(): void {
    if (this.routePoints.length < 2) {
      return;
    }

    this.currentRaceDistanceM = 0;
    this.updateDistanceMarkers();
  }
